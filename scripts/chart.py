...
def planet_in_house(planet_lon, asc_lon, house_cusps):
    ...
    def from_asc(angle):
        return normalize_angle(angle - asc_n)

    p_from_asc = from_asc(p_n)
    for i in range(12):
        cusp_from_asc = from_asc(hc_n[i])
        next_from_asc = from_asc(hc_n[(i + 1) % 12])
        if next_from_asc > cusp_from_asc:
            in_range = cusp_from_asc <= p_from_asc < next_from_asc
        else:
            in_range = p_from_asc >= cusp_from_asc or p_from_asc < next_from_asc
        if in_range:
            return i + 1
    return 12
...
def calculate_chart(birth_date, birth_time, precision, lat, lon, tz_name):
    """
    精度过滤规则：
      exact:     5行星 + ASC/MC/houses（需有效经纬度）+ 相位
      estimated: 5行星 + ASC/MC（需有效经纬度），无相位无宫位
      approximate: 仅太阳
      unknown:   仅太阳
    经纬度未提供时 ascendant/midheaven/house_cusps 不输出。
    """
    ...
    for name in included_planets:
        raw = planet_raw[name]
        symbol, sign_name, degree, idx = lon_to_sign(raw["lon"])
...
def main():
    ...
    parser.add_argument(
        ...
        help="对照测试模式：输出基准数据，与成熟排盘软件逐项核对"
    )
    ...
    if args.reference_test:
        ...
        for name, raw in planet_raw.items():
            symbol, sign_name, degree, idx = lon_to_sign(raw["lon"])
            planets[name] = {
                "sign": symbol, "sign_name": sign_name, "sign_index": idx,
                "degree": degree, "ecliptic_lon": raw["lon"], "ecliptic_lat": raw["lat"],
            }
        result = {
            "mode": "reference_test",
            "birth": {"date": "2002-08-16", "time": "11:30", "location": "安徽省六安市霍邱县",
